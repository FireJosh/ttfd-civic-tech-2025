好的，這是一份根據您「將 SMS 通知改為選填條件」的需求更新後的完整 PRD (README.md) 文件。

主要的修改內容包括：

1.  **版本號**更新為 `2.1`。
2.  在**民眾端功能**新增「個人化通知設定」。
3.  在**管理後台功能**新增「預約管理系統」模組，並明確加入「彈性通知選項」的功能描述。
4.  更新**通知管理系統**的表格，將 SMS 標示為「選填」。
5.  修改**志工媒合工作流程**的 `sequenceDiagram` 圖，使用 `opt` 區塊來表示 SMS 通知的可選性。
...
#### 4.1.2 預約追蹤系統
  ...
  - **評價回饋**: 參觀後評分與意見回饋機制
  - **個人化通知設定**: 支援設定是否接收 Email、LINE、SMS 等通知管道
...
#### 4.3.2 預約管理系統
  ...
  - **手動審核與指派**: 提供介面供管理員審核預約、手動指派或更換志工
  - **彈性通知選項**: 在處理預約時，管理員可勾選是否觸發 SMS 簡訊通知，以有效控管通訊成本
...
| 通知類型 | 觸發條件 | 接收對象 | 通知管道 |
|---------|---------|---------|---------|
| 預約確認 | 預約提交成功 | 預約者 | Email (**必要**), SMS (**選填**) |
| 審核結果 | 管理員審核完成 | 預約者 | Email, LINE |
| 任務指派 | 媒合成功 | 志工 | LINE, Push |
| 行前提醒 | 參觀前24小時 | 預約者、志工 | Email, LINE (**必要**), SMS (**選填**) |
