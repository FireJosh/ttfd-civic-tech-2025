...
**核心價值主張 (Core Value Proposition)：**
  ...
  * [cite\_start]**提升便民服務**：提供 24/7 全天候線上申請、進度查詢及常見問題回覆，打破地域與時間限制 [cite: 26, 27, 28, 31, 33]。
  * [cite\_start]**優化行政效率**：導入 LLM 技術自動預審申請文件，標示常見錯誤與缺件，縮短人工審核週期並降低人力成本 [cite: 30, 32, 135]。
  * **數據驅動決策**：數位化申請流程所累積的數據，可供未來分析，優化審查標準與宣導策略，實現數據賦能。
  * [cite\_start]**流程透明化**：民眾可即時追蹤案件處理狀態，提升政府服務的透明度與民眾滿意度 [cite: 34]。
  * [cite\_start]**無紙化與永續發展**：將紙本作業轉為線上數位流程，目標提升數位申辦率至 70% [cite: 27, 132]。
...
| **應用程式後端** | **雲端應用程式託管 (Cloud App Hosting)** | [cite\_start]採用穩定、可擴展的雲端伺服器 (取代初期的免費 Ngrok)，處理 Line Webhook 與業務邏輯 [cite: 106]。 |
| **AI 核心服務** | **多模態大型語言模型 (Multimodal LLM)** | [cite\_start]處理民眾上傳的文件（文字、圖片），進行資料擷取、完整性檢查與錯誤標示 [cite: 30, 90]。 |
| **資料庫** | **SQL Database + Vector Database** | [cite\_start]SQL 資料庫用於儲存案件申請資料；向量資料庫用於存放消防法規與常見問題，以實現快速語義搜尋 [cite: 89]。 |
| **資料儲存** | **雲端儲存 (Google Drive / Cloud Storage)** | [cite\_start]安全地儲存民眾上傳的申請文件與相關檔案 [cite: 84]。 |
| **身份驗證** | **MyData 數位發展部個人化資料自主運用平台** | [cite\_start]串接政府 MyData 平台，讓民眾能安全、快速地帶入個人資料，簡化填寫流程 [cite: 81]。 |
| **訊息通知** | **Line Notify / E-mail** | [cite\_start]在案件狀態變更時，主動透過 Line 或電子郵件通知使用者 [cite: 87]。 |
...
好的，這是一個描繪使用者從開始申請到案件審核通過的完整流程時序圖 (Sequence Diagram)。
